<?php

header('Content-Type: application/json');
require __DIR__ . '/db.php';


$managerUserId = 'mgr01';

$leaveCode = trim($_POST['leaveId'] ?? ''); 
if ($leaveCode === '') {
  echo json_encode(['success' => false, 'message' => 'Leave Request ID is required.']);
  exit;
}

try {
  // Get manager DB id
  $stmt = $pdo->prepare("SELECT id FROM users WHERE userID = ? AND role = 'MANAGER' AND status='ACTIVE'");
  $stmt->execute([$managerUserId]);
  $mgr = $stmt->fetch();
  if (!$mgr) {
    echo json_encode(['success' => false, 'message' => 'Manager user not found or inactive.']);
    exit;
  }
  $managerId = (int)$mgr['id'];

  
  $upd = $pdo->prepare("
      UPDATE leave_requests
      SET status = 'APPROVED',
          approved_by_user_id = ?,
          approved_at = NOW()
      WHERE request_code = ? AND status = 'PENDING'
  ");
  $upd->execute([$managerId, $leaveCode]);

  if ($upd->rowCount() === 0) {
    echo json_encode(['success' => false, 'message' => "No pending leave found for '$leaveCode' or already processed."]);
  } else {
    echo json_encode(['success' => true, 'message' => "Leave Request '$leaveCode' approved successfully."]);
  }
} catch (Throwable $e) {
  echo json_encode(['success' => false, 'message' => 'Error: ' . $e->getMessage()]);
}
